...
# -*- coding: utf-8 -*-
"""
Neoenergia Selenium Worker - CELPE
- Mantém a estrutura real de índice, extrator e lógica
- Usa pasta temporária exclusiva: downloads_temp_celpe
- Não executa main sozinho
- Expõe: run_worker_celpe(jobs, shared_lock)
"""
...
try:
    ...
    def _emit(outcome: str, *, instalacao: str, ref: str, carimbo: str = "") -> None:
        _emit_neo_outcome(outcome, utility="CELPE", account_id=instalacao,
...
_ucs_alvo: Optional[Set[str]] = None  # Filtro de UCs para resgates direcionados
_ignorar_indice = False  # Resgate/redownload: ignora o índice de já baixados
...
TEMP_DOWNLOAD_DIR = BASE_DIR / "downloads_temp_celpe"
...
def _inicializar_master() -> None:
    """Chamado apenas dentro de run_worker_celpe, nunca no nível de módulo."""
...
def _normalizar_uc(codigo: str) -> str:
    """Normaliza código de UC removendo zeros à esquerda e espaços."""
...
def _handle_selecionar_perfil(driver) -> bool:
    """
    Trata a tela /selecionar-perfil-de-acesso do portal CELPE.
    Tenta clicar no card de perfil que contenha o nome do estado
    ou, na falta, no primeiro card disponível.
    Retorna True se conseguiu avançar para além dessa tela.
    """
    if "selecionar-perfil" not in driver.current_url:
        return True  # não precisa tratar

    log.info("  Tela de seleção de perfil detectada — tentando avançar")

    # Tenta primeiro um card que mencione Pernambuco / PE / CELPE
    candidatos_pref = [
        (By.XPATH, "//mat-card[contains(.,'Pernambuco') or contains(.,'CELPE') or contains(.,'PE')]"),
        (By.XPATH, "//mat-card"),
        (By.CSS_SELECTOR, "mat-card"),
    ]
    for by, sel in candidatos_pref:
        try:
            ...
            for card in cards:
                if card.is_displayed():
                    ...
                    log.info(f"  Perfil selecionado — URL: {driver.current_url}")
                    return "selecionar-perfil" not in driver.current_url
        except Exception:
            continue

    log.warning("  Não encontrou card de perfil para clicar")
    return False
...
def selecionar_faturas_pendentes(driver, ja_baixados: Set[str], cnpj: str, instalacao: str) -> List[FaturaTela]:
    ...
    if elegiveis_avencer:
        ...
        log.info(
            f"  Sem 'A VENCER'; selecionando a última fatura disponível: "
            f"{escolhida.referencia} | situacao={escolhida.situacao}"
        )
...
def processar_uc(
    ...
    if _shared_lock is not None:
        with _shared_lock:
            ...
            try:
                # Diretório criado dentro do try — se falhar, carimbo não é consumido
                estado_dir = FINAL_DOWNLOAD_ROOT / estado_slug(estado)
...
def processar_estado(driver, estado: str, cnpj: str, ja_baixados: Set[str]) -> int:
    # Se caiu em tela de perfil (sem tratar antes), resolve agora
    _handle_selecionar_perfil(driver)
    ...
    while True:
        ...
        if _ucs_alvo is not None:
            # Resgate: processa qualquer UC alvo independente de status de ligação
            ...
            log.info(f"  Página {pagina}: {len(ucs)} UC(s), {len(ligadas)} LIGADA(s), {len(alvo_pagina)} alvo(s) (filtro_uc=ativo)")
        ...
        if n_estados_esperados == 1:
            ...
            if _tela_tem_selecao_estados(driver):
                ...
                if not selecionar_estado(driver, estado_nome):
                    ...
                    return 0
                # Verifica se caiu em tela de seleção de perfil antes dos imóveis
                _handle_selecionar_perfil(driver)
                total_ok += processar_estado(driver, estado_nome, info.cnpj, ja_baixados)
                return total_ok
...
if __name__ == "__main__":
    ...
    _ap = argparse.ArgumentParser(description="Worker CELPE")
    _ap.add_argument(
        "--condicao",
    ...
    _csv_path = BASE_DIR / "cnpjs_neoenergia.csv"
    if not _csv_path.exists():
        print(f"[CELPE] CSV não encontrado: {_csv_path}")
        raise SystemExit(1)

    _estados_worker = ["Pernambuco"]
    _jobs = []
    ...
    print(f"[CELPE] {len(_jobs)} jobs carregados de {_csv_path.name}")
    if _permitir_qualquer:
        print(f"[CELPE] Condição: {_args.condicao} (todas as faturas não pagas)")
    if not _jobs:
        print(f"[CELPE] Nenhum job para {_estados_worker}. Verifique o CSV.")
        raise SystemExit(0)

    run_worker_celpe(_jobs, shared_lock=Lock(), permitir_qualquer_situacao=_permitir_qualquer)
