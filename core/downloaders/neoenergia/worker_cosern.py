...
# -*- coding: utf-8 -*-
"""
Neoenergia Selenium Worker - COSERN
- Mantém a estrutura real de índice, extrator e lógica
- Usa pasta temporária exclusiva: downloads_temp_cosern
- Não executa main sozinho
- Expõe: run_worker_cosern(jobs, shared_lock)
...
try:
    ...
    def _emit(outcome: str, *, instalacao: str, ref: str, carimbo: str = "") -> None:
        _emit_neo_outcome(outcome, utility="COSERN", account_id=instalacao,
...
COSERN_ACESSO_CANDIDATES = [
    Path(r"\\10.10.250.21\Energia\ARQUIVOS ENZO\DOWNLOAD NEOENERGIA\COSERN ACESSO.xlsx"),
    BASE_DIR / "COSERN ACESSO.xlsx",
]

LOG_DIR = BASE_DIR / "logs"
TEMP_DOWNLOAD_DIR = BASE_DIR / "downloads_temp_cosern"
...
def _inicializar_master() -> None:
    """Chamado apenas dentro de run_worker_cosern, nunca no nível de módulo."""
...
def _normalizar_estado_nome(s: str) -> str:
    txt = _fix_mojibake(s).strip().lower()
...
def processar_uc(
    ...
    if _shared_lock is not None:
        with _shared_lock:
            ...
            try:
                estado_dir = FINAL_DOWNLOAD_ROOT / estado_slug(estado)
                mes_dir = estado_dir / referencia_to_folder(f.referencia)
    ...
    while True:
        ...
        for pos, uc in enumerate(ligadas, start=1):
            ...
            if pagina > 1:
                ...
                # Aguarda a lista estabilizar antes de paginar
                try:
                    WebDriverWait(driver, 15).until(
                        EC.presence_of_all_elements_located((By.XPATH, "//div[contains(@class,'box-imoveis')]"))
                    )
                ...
                for n_pag in range(pagina - 1):
                    time.sleep(0.6)
                    if not _proxima_pagina_ucs(driver):
                        log.warning(f"  Reposicionamento falhou na paginação {n_pag + 1}/{pagina - 1} — reiniciando do início da página 1")
                        ok_paginou = False
                        break
                    time.sleep(0.5)
                if not ok_paginou:
...
def _carregar_jobs_xlsx_cosern() -> tuple[List[dict], Path | None]:
    if load_workbook is None:
        return [], None

    for candidato in COSERN_ACESSO_CANDIDATES:
        try:
            ...
            try:
                ...
                headers_norm = {_normalizar_cabecalho(h): i for i, h in enumerate(headers)}
                idx_conc = headers_norm["concessionaria"]
                idx_cnpj = headers_norm["cnpj"]
                idx_senha = headers_norm["senha"]

                jobs_map: dict[str, dict] = {}
                for row in ws.iter_rows(min_row=2, values_only=True):
                    concessionaria = str(row[idx_conc] or "").strip().upper()
                    if "COSERN" not in concessionaria and "RIO GRANDE DO NORTE" not in concessionaria:
                        continue

                    cnpj = fmt_doc(row[idx_cnpj] or "")
                    senha = str(row[idx_senha] or "").strip()
                    if cnpj and senha:
                        jobs_map[cnpj] = {
                            "cnpj": cnpj,
                            "senha": senha,
                            "estados_esperados": ["Rio Grande do Norte"],
                        }
            finally:
                wb.close()

            if jobs_map:
                return list(jobs_map.values()), candidato
        except Exception:
            continue
...
def _carregar_jobs_csv_cosern(csv_path: Path) -> List[dict]:
    estados_worker_norm = {_normalizar_estado_nome("Rio Grande do Norte")}
    jobs: List[dict] = []

    with open(csv_path, encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            estados = [e.strip() for e in _fix_mojibake(row.get("ESTADOS", "")).split("|") if e.strip()]
            estados_canon = []
            for estado in estados:
                canon = _ESTADO_CANONICO.get(_normalizar_estado_nome(estado))
                if canon:
                    estados_canon.append(canon)

            if any(_normalizar_estado_nome(e) in estados_worker_norm for e in estados_canon):
                jobs.append(
                    {
                        "cnpj": row.get("CNPJ", "").strip(),
                        "senha": row.get("SENHA", "").strip(),
                        "estados_esperados": estados_canon,
                    }
                )
...
def carregar_jobs_padrao_cosern() -> tuple[List[CnpjInfo], str]:
    jobs_xlsx, origem_xlsx = _carregar_jobs_xlsx_cosern()
    if jobs_xlsx:
        return _ordenar_cnpjs(_coletar_jobs(jobs_xlsx)), str(origem_xlsx)

    csv_path = BASE_DIR / "cnpjs_neoenergia.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"CSV não encontrado: {csv_path}")

    jobs_csv = _carregar_jobs_csv_cosern(csv_path)
    return _ordenar_cnpjs(_coletar_jobs(jobs_csv)), str(csv_path)
...
def run_worker_cosern(
    ...
    from core.downloaders.neoenergia.worker_coelba import run_worker_coelba as _run_worker_portal

    jobs_normalizados = [
        {
            "cnpj": item.cnpj,
            "senha": item.senha,
            "estados_esperados": list(item.estados_esperados or []),
        }
        for item in _coletar_jobs(jobs)
    ]

    return _run_worker_portal(
        jobs_normalizados,
        shared_lock=shared_lock,
        progress_queue=progress_queue,
        ucs_alvo=ucs_alvo,
        ignorar_indice=ignorar_indice,
        baixar_todas_faturas_ano=baixar_todas_faturas_ano,
        ano_alvo=ano_alvo,
        destino_subpasta=destino_subpasta,
        permitir_qualquer_situacao=permitir_qualquer_situacao,
        permitir_qualquer_ano=permitir_qualquer_ano,
        refs_alvo=refs_alvo,
        pagina_inicial_ucs=pagina_inicial_ucs,
        skip_alvos_iniciais=skip_alvos_iniciais,
        usar_pesquisa_direta=False,
        worker_name="cosern",
    )
...
if __name__ == "__main__":
    ...
    _ap = argparse.ArgumentParser(description="Worker COSERN")
    _ap.add_argument(
        "--condicao",
    ...
    try:
        _jobs, _origem = carregar_jobs_padrao_cosern()
    except FileNotFoundError as exc:
        print(f"[COSERN] {exc}")
        raise SystemExit(1)

    print(f"[COSERN] {len(_jobs)} jobs carregados de {Path(_origem).name}")
    if not _jobs:
        print("[COSERN] Nenhum job para Rio Grande do Norte. Verifique as fontes de acesso.")
        raise SystemExit(0)

    if _permitir_qualquer:
        print(f"[COSERN] Condição: {_args.condicao} (todas as faturas não pagas)")
    run_worker_cosern(_jobs, shared_lock=Lock(), permitir_qualquer_situacao=_permitir_qualquer)
