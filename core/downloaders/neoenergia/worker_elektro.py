...
# -*- coding: utf-8 -*-
"""
Neoenergia Selenium Worker - ELEKTRO
- Mantém a estrutura real de índice, extrator e lógica
- Usa pasta temporária exclusiva: downloads_temp_elektro
- Não executa main sozinho
- Expõe: run_worker_elektro(jobs, shared_lock)
...
try:
    ...
    def _emit(outcome: str, *, instalacao: str, ref: str, carimbo: str = "") -> None:
        _emit_neo_outcome(outcome, utility="ELEKTRO", account_id=instalacao,
...
TEMP_DOWNLOAD_DIR = BASE_DIR / "downloads_temp_elektro"
...
def _inicializar_master() -> None:
    """Chamado apenas dentro de run_worker_elektro, nunca no nível de módulo."""
...
if __name__ == "__main__":
    ...
    _ap = argparse.ArgumentParser(description="Worker ELEKTRO")
    _ap.add_argument(
        "--condicao",
    ...
    if not _csv_path.exists():
        print(f"[ELEKTRO] CSV não encontrado: {_csv_path}")
        raise SystemExit(1)

    _estados_worker = ["São Paulo", "Mato Grosso do Sul"]
    _estados_worker_norm = {_normalizar_estado_nome(e) for e in _estados_worker}
    _jobs = []
    with open(_csv_path, encoding="utf-8-sig") as _f:
        for _row in _csv.DictReader(_f):
            _estados = [e.strip() for e in _fix_mojibake(_row.get("ESTADOS", "")).split("|") if e.strip()]
            _estados_canon = []
            for e in _estados:
                canon = _ESTADO_CANONICO.get(_normalizar_estado_nome(e))
                if canon:
                    _estados_canon.append(canon)
            if any(_normalizar_estado_nome(e) in _estados_worker_norm for e in _estados_canon):
                _jobs.append({
                    "cnpj": _row["CNPJ"].strip(),
                    "senha": _row["SENHA"].strip(),
                    "estados_esperados": _estados_canon,
                })

    print(f"[ELEKTRO] {len(_jobs)} jobs carregados de {_csv_path.name}")
    if not _jobs:
        print(f"[ELEKTRO] Nenhum job para {_estados_worker}. Verifique o CSV.")
        raise SystemExit(0)

    if _permitir_qualquer:
        print(f"[ELEKTRO] Condição: {_args.condicao} (todas as faturas não pagas)")
    run_worker_elektro(_jobs, shared_lock=Lock(), permitir_qualquer_situacao=_permitir_qualquer)
