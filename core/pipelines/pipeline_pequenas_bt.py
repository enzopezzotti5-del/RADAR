...
# -*- coding: utf-8 -*-
"""
Pipeline PEQUENAS BT: OCR -> Digitacao -> Filtro.
"""
...
from indice_master import MasterIndice  # noqa: E402
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_pequenas_bt.py"
...
OCR_SAIDA_DIR  = SERVIDOR / "ARQUIVOS ENZO" / "OCR PEQUENAS"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "PEQUENAS_pipeline_saida"
STAGING_DIR    = SERVIDOR / "ARQUIVOS ENZO" / "pequenas_bt_staging"
DIGITADAS_DIR  = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "PEQUENAS BT"
...
# Perfis excluídos do pipeline (fluxo separado)
EXCLUIR_PROFILES: set[str] = {"CERBRANORTE"}


def _filtrar_xlsx_excluir_profiles(xlsx: Path) -> int:
    """Remove linhas cujo SISTEMA_ORIGEM está em EXCLUIR_PROFILES. Retorna nº de linhas removidas."""
    if not xlsx.exists() or not EXCLUIR_PROFILES:
        return 0
    ...
    for i, h in enumerate(header):
        if h.upper() in ("SISTEMA_ORIGEM", "SISTEMA", "CONCESSIONARIA", "CONCCOD"):
            col_idx = i + 1  # 1-based
            break
    if col_idx is None:
        return 0
    ...
    for row in ws.iter_rows(min_row=2):
        val = str(row[col_idx - 1].value or "").strip().upper()
        if val in {p.upper() for p in EXCLUIR_PROFILES}:
...
def etapa_carimbo(pasta_origem: Path, mes: str, ano: str) -> tuple[Path, list[str]]:
    """Atribui BB_ carimbos aos PDFs e copia para staging. Retorna (staging, lista_carimbos)."""
    _mkdir_seguro(STAGING_DIR)
    pdfs = sorted(p for p in pasta_origem.glob("*.pdf") if p.is_file())
    if not pdfs:
        _fail(f"Nenhum PDF em {pasta_origem}")
        return STAGING_DIR, []
    ...
    for pdf in pdfs:
        # Reutiliza carimbo se o stem já for BB_XXXXXX
        m_bb = re.search(r"[Bb][Bb]_(\d+)", pdf.stem)
        if m_bb:
            carimbo = f"BB_{m_bb.group(1)}"
            _info(f"  {pdf.name} -> {carimbo} (reutilizado)")
        else:
            carimbo = master.consumir_carimbo()
            uc = re.split(r"\s", pdf.stem)[0]  # "19.993.067-00 10.06" -> "19.993.067-00"
            master.registrar(
                indice_bb=carimbo,
                sistema="PEQUENAS_BT",
                uc=uc,
                mes_ref=mes_ref,
                arquivo=f"{carimbo}.pdf",
            )
            _info(f"  {pdf.name} -> {carimbo}")

        destino = STAGING_DIR / f"{carimbo}.pdf"
        if not destino.exists():
            shutil.copy2(pdf, destino)
        carimbos.append(carimbo)

    _ok(f"Carimbo: {len(carimbos)} PDF(s) em staging")
    return STAGING_DIR, carimbos
...
def etapa_filtro(root_pdfs: Path, pasta_saida: Path) -> int:
    ...
    env_extra = {
        ...
        "NEO_FILTRO_DESTINO_EXISTENTES": str(DIGITADAS_DIR.parent / "Ja_existiam_no_Consen"),
    }
...
def main() -> int:
    ...
    carimbos_arg = [str(c).strip() for c in (args.carimbo or []) if str(c).strip()]
    pasta_origem = Path(str(args.pasta).strip()) if str(args.pasta).strip() else Path("")
    xlsx = OCR_SAIDA_DIR / f"ocr_pequenas_BT_{mes}{ano}.xlsx"
    ...
    _banner(f"PIPELINE {PIPELINE_NOME}", [f"Referencia : {mes}/{ano}", f"Origem     : {pasta_origem or '(default)'}"])

    # -- 1. CARIMBO ----------------------------------------------------------
    falhas: list[str] = []
    if not args.so_digitacao and not args.so_filtro and not args.so_ocr:
        if not pasta_origem or not pasta_origem.exists():
            _fail(f"Pasta de origem nao encontrada: {pasta_origem}")
            return 1
        _, carimbos = etapa_carimbo(pasta_origem, mes, ano)
        if not carimbos:
            _fail("Nenhum PDF processado na etapa de carimbo")
            return 1
        pasta_pdfs = STAGING_DIR
    else:
        # Modo debug: usa staging existente ou pasta_origem passada
        if STAGING_DIR.exists() and any(STAGING_DIR.glob("BB_*.pdf")):
            pasta_pdfs = STAGING_DIR
            carimbos = [p.stem for p in sorted(STAGING_DIR.glob("BB_*.pdf"))]
            _info(f"[debug] Usando staging existente: {len(carimbos)} PDFs")
        else:
            pasta_pdfs = pasta_origem
            carimbos = carimbos_arg
    ...
    if not args.so_ocr and not args.so_filtro:
        if not xlsx.exists():
            ...
            return 1
        removidos = _filtrar_xlsx_excluir_profiles(xlsx)
        if removidos:
            _info(f"[filtro] {removidos} linha(s) excluidas (perfis: {', '.join(EXCLUIR_PROFILES)})")
