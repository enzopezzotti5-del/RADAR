...
# -*- coding: utf-8 -*-
"""
Pipeline Neoenergia: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_neoenergia.py
    python pipeline_neoenergia.py --mes 03 --ano 2026
    python pipeline_neoenergia.py --so-ocr
    python pipeline_neoenergia.py --so-digitacao
    python pipeline_neoenergia.py --so-filtro
"""
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR NEOENERGIA" / "ELEKTRO"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "NEOENERGIA_ELEKTRO_pipeline_saida"
DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "NEOENERGIA ELEKTRO"
...
def _pasta_default(mes: str, ano: str) -> Path:
    return DOWNLOAD_ROOT / "SAO_PAULO" / f"{ano}-{mes}"
...
def _xlsx_saida(mes: str, ano: str, tipo: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_neoenergia_elektro_{tipo.upper()}_{mes}{ano}.xlsx"


def _xlsx_resgate(tipo: str, slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_neoenergia_elektro_{tipo.upper()}_{slug}.xlsx"
...
def _materializar_xlsx_elektro(mes: str, ano: str, tipo: str, destino_override: Path | None = None) -> Path:
    ...
    try:
        ...
        _warn(f"Falha ao copiar XLSX {tipo} para pasta da Elektro: {exc}")
        return origem


def _validar_xlsx_elektro(xlsx: Path, manifesto: Path, tipo: str) -> tuple[bool, str]:
    """Valida que fatCarimbo no XLSX bate com o carimbo esperado no manifesto."""
    import csv as _csv
    try:
    ...
    if manifesto.exists():
        for enc in ("utf-8-sig", "utf-8", "latin-1"):
            try:
                with manifesto.open("r", newline="", encoding=enc) as f:
                    for row in _csv.DictReader(f):
                        arq = str(row.get("arquivo_carimbado", "")).strip()
                        car = str(row.get("carimbo", "")).strip()
                        if arq and car:
                            esperado[arq] = car
                break
    ...
    wb = _openpyxl.load_workbook(xlsx, read_only=True, data_only=True)
    ws = wb.active
    headers = [str(c.value or "").strip() for c in next(ws.iter_rows(min_row=1, max_row=1))]
    try:
        idx_arquivo = headers.index("ARQUIVO")
        idx_carimbo = headers.index("fatCarimbo")
    except ValueError:
        return True, ""

    for row in ws.iter_rows(min_row=2, values_only=True):
        arquivo = str(row[idx_arquivo] or "").strip()
        carimbo = str(row[idx_carimbo] or "").strip()
        if arquivo in esperado and carimbo != esperado[arquivo]:
            return False, (
                f"fatCarimbo truncado ou errado em {tipo}: arquivo={arquivo!r} "
                f"xlsx={carimbo!r} esperado={esperado[arquivo]!r}"
            )
    return True, ""
...
def parse_args():
    hoje = dt.date.today()
    p = argparse.ArgumentParser(description="Pipeline Neoenergia Elektro: OCR -> Digitacao -> Filtro")
    ...
    p.add_argument(
        "--pasta",
        type=str,
        default=str(DOWNLOAD_ROOT / "SAO_PAULO" / f"{hoje.year}-{hoje.month:02d}"),
        help="Pasta mensal da Neoenergia com PDFs misturados",
    )
    p.add_argument(
        "--carimbo",
        action="append",
    ...
    if not args.so_ocr and not args.so_filtro:
        ...
        for tipo, xlsx in xlsxs.items():
            if not xlsx.exists() and not resgate:
                xlsx = _materializar_xlsx_elektro(mes, ano, tipo, destino_override=xlsx)
                xlsxs[tipo] = xlsx
            if not xlsx.exists():
