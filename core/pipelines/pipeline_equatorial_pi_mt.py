...
# -*- coding: utf-8 -*-
"""
Pipeline EQUATORIAL PI MT: OCR -> Digitação -> Filtro.

Uso:
    python pipeline_equatorial_pi_mt.py --pasta "\\\\srv\\...\\MT" --mes 05 --ano 2026
    python pipeline_equatorial_pi_mt.py --so-ocr --pasta "..."
"""
...
OCR_SCRIPT       = LOCAL_DIR / "ocr" / "ocr_equatorial_pi_mt.py"
...
OCR_SAIDA_DIR    = SERVIDOR / "ARQUIVOS ENZO" / "OCR EQUATORIAL PI"
PIPELINE_SAIDA   = SERVIDOR / "ARQUIVOS ENZO" / "EQUATORIAL_PI_pipeline_saida"
DIGITADAS_DIR    = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME    = "EQUATORIAL PI MT"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_equatorial_pi_MT_{mes}{ano}.xlsx"


def _pipeline_saida_dir() -> Path:
    return PIPELINE_SAIDA / "MT"
...
def etapa_ocr(mes: str, ano: str, pasta: str, xlsx_saida: Path) -> int:
    if not OCR_SCRIPT.exists():
        _fail(f"Script OCR não encontrado: {OCR_SCRIPT}")
        return 1
...
def etapa_digitacao(xlsx: Path, pasta_saida: Path) -> int:
    if not DIGITACAO_SCRIPT.exists() or not xlsx.exists():
        _fail("Script ou planilha não encontrado")
        return 1
    ...
    return _rodar_visual(
        f"DIGITAÇÃO {PIPELINE_NOME} ({xlsx.name})",
...
def etapa_renomear_pdfs(root_pdfs: Path, pasta_saida: Path) -> int:
    """Renomeia PDFs de instalação-nome para BB_XXXXXXX.pdf usando o auditoria CSV."""
    auditoria = pasta_saida / "auditoria_resultados.csv"
    if not auditoria.exists():
        _warn("auditoria não encontrada — renomeação pulada")
        return 0
        ...
        try:
            with auditoria.open("r", newline="", encoding=enc) as f:
                for row in csv.DictReader(f, delimiter=";"):
                    ...
                    if inst and car:
                        # normaliza instalação para comparação: só dígitos e traço final
                        mapa[inst] = car
            break
        except UnicodeDecodeError:
            continue
    ...
    if not mapa:
        _warn("Mapa instalação→carimbo vazio")
        return 0
    ...
    for pdf in sorted(root_pdfs.glob("*.pdf")):
        stem = pdf.stem
        # Remove sufixo de data: "1.149.960.019-97 - 20.06" → "1.149.960.019-97"
        instalacao_fmt = _re.sub(r"\s*-+\s*\d{1,2}\.\d{2}\s*$", "", stem).strip()
        carimbo = mapa.get(instalacao_fmt)
        if not carimbo:
            # fallback: dígitos-only match
        ...
        if novo_nome.exists():
            _warn(f"Já existe: {novo_nome.name} — pulando {pdf.name}")
            continue
        try:
            pdf.rename(novo_nome)
            renomeados += 1
            _info(f"Renomeado: {pdf.name} → {novo_nome.name}")
        except Exception as exc:
            _warn(f"Erro ao renomear {pdf.name}: {exc}")
            nao_encontrados += 1

    _ok(f"Renomeação: {renomeados} PDFs renomeados, {nao_encontrados} não encontrados")
    return 0


def etapa_filtro(root_pdfs: Path, pasta_saida: Path) -> int:
    if not FILTRO_SCRIPT.exists():
        _fail(f"Script de filtro não encontrado: {FILTRO_SCRIPT}")
        return 1
    auditoria = pasta_saida / "auditoria_resultados.csv"
    if not auditoria.exists():
        _warn("auditoria não encontrada — filtro pulado")
        return 0
    env_extra = {
...
def main() -> int:
    ...
    xlsx = _xlsx_saida(mes, ano)
    pasta_saida = _pipeline_saida_dir()
    ...
    if not args.so_digitacao and not args.so_filtro:
        if not pasta_pdfs.exists():
            _fail(f"Pasta não encontrada: {pasta_pdfs}"); return 1
    ...
    _ok(f"PIPELINE {PIPELINE_NOME} CONCLUÍDO COM SUCESSO"); _sep("=")
    return 0
