...
# -*- coding: utf-8 -*-
"""
Pipeline ENERGISA BT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_energisa_bt.py
    python pipeline_energisa_bt.py --mes 05 --ano 2026 --pasta "//servidor/ENERGISA/2026-05"
    python pipeline_energisa_bt.py --so-ocr
    python pipeline_energisa_bt.py --so-digitacao
    python pipeline_energisa_bt.py --so-filtro
"""
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_energisa_bt.py"
...
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "ENERGISA_pipeline_saida"
DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "ENERGISA BT"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_energisa_BT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_energisa_BT_{slug}.xlsx"
