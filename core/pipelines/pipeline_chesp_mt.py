...
# -*- coding: utf-8 -*-
"""
Pipeline CHESP MT (A4 Verde/Azul): Carimbo -> OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_chesp_mt.py --pasta "\\\\srv\\...\\CHESP\\MT" --mes 07 --ano 2026
    python pipeline_chesp_mt.py --so-ocr
    python pipeline_chesp_mt.py --so-digitacao
"""
...
OCR_SCRIPT       = LOCAL_DIR / "ocr" / "ocr_chesp.py"
...
OCR_SAIDA_DIR  = SERVIDOR / "ARQUIVOS ENZO" / "OCR CHESP"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "CHESP_pipeline_saida"
STAGING_DIR    = SERVIDOR / "ARQUIVOS ENZO" / "chesp_mt_staging"
DIGITADAS_DIR  = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME  = "CHESP MT"

PASTA_PDF_DEFAULT = Path(
    "//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Faturas/CHESP/MT"
)
...
def etapa_ocr(mes: str, ano: str, pasta: str, xlsx_saida: Path, carimbos: list[str]) -> int:
    ...
    cmd = [PYTHON_EXE, str(OCR_SCRIPT), "--mes", str(int(mes)), "--ano", str(int(ano)),
           "--pasta", pasta, "--saida", str(xlsx_saida)]
    for c in carimbos:
...
def parse_args() -> argparse.Namespace:
    hoje = dt.date.today()
    p = argparse.ArgumentParser(description=f"Pipeline {PIPELINE_NOME}: Carimbo -> OCR -> Digitacao -> Filtro")
...
def main() -> int:
    ...
    pasta_origem  = Path(str(args.pasta).strip()) if str(args.pasta).strip() else PASTA_PDF_DEFAULT
    xlsx          = OCR_SAIDA_DIR / f"ocr_chesp_MT_{mes}{ano}.xlsx"
