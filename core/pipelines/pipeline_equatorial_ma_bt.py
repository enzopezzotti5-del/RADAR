...
# -*- coding: utf-8 -*-
"""
Pipeline EQUATORIAL MA BT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_equatorial_ma_bt.py --mes 06 --ano 2026 --pasta "//servidor/..."
"""
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_equatorial_ma_bt.py"
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR EQUATORIAL MA"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "EQUATORIAL_MA_pipeline_saida"
DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "EQUATORIAL MA BT"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_equatorial_ma_BT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_equatorial_ma_BT_{slug}.xlsx"
...
def main() -> int:
    ...
    auditoria_saida = Path(args.auditoria_saida.strip()) if args.auditoria_saida and args.auditoria_saida.strip() else None
    ...
    if not args.so_ocr and not args.so_filtro:
        ...
        # Copiar auditoria para caminho explícito solicitado pelo chamador
        ...
        if auditoria_saida and auditoria_interna.exists():
            ...
            _shutil.copy2(str(auditoria_interna), str(auditoria_saida))
