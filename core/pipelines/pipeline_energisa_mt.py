...
# -*- coding: utf-8 -*-
"""
Pipeline ENERGISA MT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_energisa_mt.py --pasta "//servidor/Teste MT ENERGISA" --mes 6 --ano 2026
    python pipeline_energisa_mt.py --pasta "..." --carimbo 971303 --carimbo 971304
    python pipeline_energisa_mt.py --so-ocr
    python pipeline_energisa_mt.py --so-digitacao
    python pipeline_energisa_mt.py --so-filtro
"""
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_energisa_mt.py"
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR ENERGISA"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "ENERGISA_MT_pipeline_saida"
DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "ENERGISA MT"
...
def _extrair_uc_do_nome(pdf: Path) -> str:
    nome = pdf.stem.strip()
    if " - " in nome:
...
def _inferir_estado_energisa(pasta: Path) -> str:
    path_norm = str(pasta).upper()
    mapa = {
        "PARAIBA": "PB",
...
def _etapa_carimbo(pasta: Path, mes: str, ano: str) -> None:
    ...
    estado = _inferir_estado_energisa(pasta)
    _info(f"[carimbo] {len(nao_bb)} PDF(s) sem carimbo - atribuindo via indice master...")
    try:
        sys.path.insert(0, str(LOCAL_DIR.parent))
        try:
            from scripts.infra.indice_master import MasterIndice
        except ModuleNotFoundError:
            from indice_master import MasterIndice

        master = MasterIndice()
        for pdf in nao_bb:
            uc = _extrair_uc_do_nome(pdf)
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_energisa_MT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_energisa_MT_{slug}.xlsx"


def _pipeline_saida_dir(slug: str = "") -> Path:
    return PIPELINE_SAIDA / "_resgates" / slug if slug else PIPELINE_SAIDA / "MT"
...
def _atualizar_master(auditoria_csv: Path) -> None:
    try:
        sys.path.insert(0, str(LOCAL_DIR.parent))
        try:
            from scripts.infra.indice_master import MasterIndice, marcar_digitados_do_auditoria
        except ModuleNotFoundError:
...
def main() -> int:
    ...
    if not args.so_digitacao and not args.so_filtro:
        if not pasta_pdfs or not pasta_pdfs.exists():
            ...
            return 1
        _etapa_carimbo(pasta_pdfs, mes, ano)
