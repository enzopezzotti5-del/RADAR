...
# -*- coding: utf-8 -*-
"""
Pipeline EQUATORIAL AL BT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_equatorial_al_bt.py --mes 06 --ano 2026 --pasta "//servidor/..."
"""
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_equatorial_al_bt.py"
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR EQUATORIAL AL"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "EQUATORIAL_AL_pipeline_saida"
DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "EQUATORIAL AL BT"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_equatorial_al_BT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_equatorial_al_BT_{slug}.xlsx"
...
def _publicar_auditoria_sessao(
    ...
    """Copia auditoria_resultados.csv da sessão para caminho explícito do lote.

    A cópia só ocorre quando o CSV pertence à sessão atual: quando carimbos foram
    informados, todos precisam aparecer no arquivo. Isso evita usar auditoria
    antiga ou de outro lote apenas por ser o arquivo mais recente.
    """
    if auditoria_saida is None:
        return True
    origem = pasta_saida / "auditoria_resultados.csv"
    if not origem.exists():
        _warn(f"auditoria_resultados.csv nao encontrado para publicar: {origem}")
        return False

    esperados = {c.replace("BB_", "").strip() for c in carimbos if c.strip()}
    if esperados:
        texto = origem.read_text(encoding="utf-8-sig", errors="replace")
        ausentes = sorted(c for c in esperados if c not in texto)
        if ausentes:
            _warn(
                "auditoria_resultados.csv nao corresponde a sessao; "
                f"carimbos ausentes: {', '.join(ausentes)}"
            )
            return False

    auditoria_saida.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(origem, auditoria_saida)
...
def main() -> int:
    ...
    if not args.so_ocr:
        if not _publicar_auditoria_sessao(pasta_saida, auditoria_saida, carimbos):
            falhas.append("AUDITORIA_SESSAO")
