...
# -*- coding: utf-8 -*-
"""
Pipeline EQUATORIAL MA MT: OCR -> Digitacao -> Renomear -> Filtro.

Uso:
    python pipeline_equatorial_ma_mt.py --pasta "\\\\srv\\...\\MT" --mes 06 --ano 2026
    python pipeline_equatorial_ma_mt.py --so-ocr --pasta "..."
"""
...
OCR_SCRIPT       = LOCAL_DIR / "ocr" / "ocr_equatorial_ma_mt.py"
...
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "EQUATORIAL_MA_MT_pipeline_saida"
DIGITADAS_DIR  = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME  = "EQUATORIAL MA MT"
...
PASTA_PDF_DEFAULT = Path(
    "//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Faturas/EQUATORIAL/MARANHÃO/MT"
)
...
def etapa_renomear_pdfs(root_pdfs: Path, pasta_saida: Path) -> int:
    """Renomeia PDFs de instalacao-nome para BB_XXXXXXX.pdf usando o auditoria CSV."""
    auditoria = pasta_saida / "auditoria_resultados.csv"
    if not auditoria.exists():
    ...
    for pdf in sorted(root_pdfs.glob("*.pdf")):
        stem = pdf.stem
        # Remove sufixo de data e qualquer texto após: "829.435.016-23 - 26.07 media" → "829.435.016-23"
        instalacao_fmt = _re.sub(r"\s*-+\s*\d{1,2}\.\d{2}.*$", "", stem).strip()
...
def main() -> int:
    ...
    xlsx        = OCR_SAIDA_DIR / f"ocr_equatorial_ma_MT_{mes}{ano}.xlsx"
