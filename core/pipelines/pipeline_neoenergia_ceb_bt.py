...
# -*- coding: utf-8 -*-
"""
Pipeline Neoenergia Brasília (CEB) BT: OCR -> Digitação -> Filtro.

Uso:
    python pipeline_neoenergia_ceb_bt.py --pasta "\\\\srv\\...\\CEB\\BT" --mes 07 --ano 2026
    python pipeline_neoenergia_ceb_bt.py --so-ocr
    python pipeline_neoenergia_ceb_bt.py --so-digitacao
"""
...
OCR_SCRIPT       = LOCAL_DIR / "ocr" / "ocr_ceb_bt.py"
...
OCR_SAIDA_DIR  = SERVIDOR / "ARQUIVOS ENZO" / "OCR CEB"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "CEB_pipeline_saida"
DIGITADAS_DIR  = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME  = "NEOENERGIA CEB BT"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_ceb_BT_{mes}{ano}.xlsx"


def _pipeline_saida_dir() -> Path:
    return PIPELINE_SAIDA / "BT"
