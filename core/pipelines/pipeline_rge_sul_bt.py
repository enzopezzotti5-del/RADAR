...
Pipeline RGE SUL BT: OCR -> Digitacao -> Filtro.



Uso:

    python pipeline_rge_sul_bt.py

    python pipeline_rge_sul_bt.py --mes 05 --ano 2026 --pasta "//servidor/RGE/2026-05"

    python pipeline_rge_sul_bt.py --so-ocr

    python pipeline_rge_sul_bt.py --so-digitacao

    python pipeline_rge_sul_bt.py --so-filtro
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_rge_sul_bt.py"
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR RGE SUL"

PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "RGE_SUL_pipeline_saida"

DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")

PIPELINE_NOME = "RGE SUL BT"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    ...
    return OCR_SAIDA_DIR / f"ocr_rge_sul_BT_{mes}{ano}.xlsx"
...
def _xlsx_resgate(slug: str) -> Path:
    ...
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_rge_sul_BT_{slug}.xlsx"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--auditoria-saida", type=str, default="",
                   help="Caminho explícito onde copiar auditoria_resultados.csv após digitação")
...
def main() -> int:
    ...
    auditoria_saida = Path(str(args.auditoria_saida).strip()) if str(args.auditoria_saida).strip() else None
    ...
    if not args.so_ocr and not args.so_filtro:
        ...
        auditoria_interna = pasta_saida / "auditoria_resultados.csv"

        if auditoria_saida and auditoria_interna.exists():

            auditoria_saida.parent.mkdir(parents=True, exist_ok=True)

            shutil.copy2(str(auditoria_interna), str(auditoria_saida))

            _info(f"Auditoria copiada para: {auditoria_saida}")
