...
# -*- coding: utf-8 -*-
"""
Pipeline CPFL MT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_cpfl_mt.py
    python pipeline_cpfl_mt.py --mes 05 --ano 2026 --pasta "//servidor/CPFL/MT/05-2026"
    python pipeline_cpfl_mt.py --so-ocr
    python pipeline_cpfl_mt.py --so-digitacao
    python pipeline_cpfl_mt.py --so-filtro
"""
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_cpfl_mt.py"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_cpfl_MT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_cpfl_MT_{slug}.xlsx"
