...
Pipeline Neoenergia Pernambuco (CELPE): OCR -> Digitacao -> Filtro.



Uso:

    python pipeline_neoenergia_pernambuco.py

        python pipeline_neoenergia_pernambuco.py --mes 03 --ano 2026



    python pipeline_neoenergia_pernambuco.py --so-ocr

    python pipeline_neoenergia_pernambuco.py --so-digitacao

    python pipeline_neoenergia_pernambuco.py --so-filtro
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR NEOENERGIA" / "PERNAMBUCO"

PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "NEOENERGIA_PERNAMBUCO_pipeline_saida"

DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")

PIPELINE_NOME = "NEOENERGIA PERNAMBUCO"
...
def _pasta_default(mes: str, ano: str) -> Path:
    ...
    return DOWNLOAD_ROOT / "PERNAMBUCO" / f"{ano}-{mes}"
...
def _xlsx_saida(mes: str, ano: str, tipo: str) -> Path:
    ...
    return OCR_SAIDA_DIR / f"ocr_neoenergia_pernambuco_{tipo.upper()}_{mes}{ano}.xlsx"
...
def _xlsx_resgate(tipo: str, slug: str) -> Path:
    ...
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_neoenergia_pernambuco_{tipo.upper()}_{slug}.xlsx"
...
def _materializar_xlsx_pernambuco(mes: str, ano: str, tipo: str, destino_override: Path | None = None) -> Path:
    ...
    try:
        ...
        _warn(f"Falha ao copiar XLSX {tipo} para pasta de Pernambuco: {exc}")
...
def parse_args():
    ...
    p = argparse.ArgumentParser(description="Pipeline Neoenergia Pernambuco: OCR -> Digitacao -> Filtro")
    ...
    p.add_argument(
        ...
        help="Pasta mensal da Neoenergia Pernambuco com PDFs misturados",
    ...
    p.add_argument(
        ...
        help="Carimbo(s) especifico(s) para OCR (pode repetir). Ex: --carimbo BB_2001037",
    ...
    if not args.so_ocr and not args.so_filtro:
        ...
        for tipo, xlsx in xlsxs.items():
            ...
            if not xlsx.exists() and not resgate:
                ...
                xlsx = _materializar_xlsx_pernambuco(mes, ano, tipo, destino_override=xlsx)
