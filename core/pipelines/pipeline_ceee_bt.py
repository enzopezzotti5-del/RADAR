...
# -*- coding: utf-8 -*-
"""
Pipeline CEEE BT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_ceee_bt.py
    python pipeline_ceee_bt.py --mes 05 --ano 2026 --pasta "//servidor/CEEE/2026-05"
    python pipeline_ceee_bt.py --so-ocr
    python pipeline_ceee_bt.py --so-digitacao
    python pipeline_ceee_bt.py --so-filtro
"""
...
OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_ceee_bt.py"
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR CEEE"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "CEEE_pipeline_saida"
DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "CEEE BT"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_ceee_BT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_ceee_BT_{slug}.xlsx"
