...
# -*- coding: utf-8 -*-
"""
Pipeline Light RJ BT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_light_bt.py
    python pipeline_light_bt.py --mes 05 --ano 2026 --pasta "//servidor/LIGHT"
    python pipeline_light_bt.py --so-ocr
    python pipeline_light_bt.py --so-digitacao
    python pipeline_light_bt.py --so-filtro
"""
...
OCR_SCRIPT       = LOCAL_DIR / "ocr"              / "ocr_light_rj_bt.py"
...
OCR_SAIDA_DIR  = SERVIDOR / "ARQUIVOS ENZO" / "OCR LIGHT RJ"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "LIGHT_pipeline_saida"
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_light_rj_BT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_light_rj_BT_{slug}.xlsx"
...
def etapa_digitacao(xlsx: Path, pasta_pdfs: Path, pasta_saida: Path) -> int:
    ...
    return _rodar_visual(f"DIGITACAO {PIPELINE_NOME} ({xlsx.name})",
                         [PYTHON_EXE, str(DIGITACAO_SCRIPT)], env_extra=env_extra)
...
def etapa_filtro(root_pdfs: Path, pasta_saida: Path) -> int:
    ...
    codigo = _rodar_visual(f"FILTRO {PIPELINE_NOME}",
                           [PYTHON_EXE, str(FILTRO_SCRIPT)], env_extra=env_extra)
...
def main() -> int:
    ...
    if not args.so_ocr and not args.so_filtro:
        ...
        cod = etapa_digitacao(xlsx, pasta_pdfs, pasta_saida)
        ...
        if cod != 0:
            if resumo["moviveis"] > 0:
                _warn(f"Digitacao terminou com exit {cod}, mas ha {resumo['moviveis']} "
                      f"conta(s) aptas ao filtro. Continuando.")
            else:
