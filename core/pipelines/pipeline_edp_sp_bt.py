...
# -*- coding: utf-8 -*-
"""
Pipeline EDP SP BT: OCR -> Digitacao -> Filtro.

Uso:
    python pipeline_edp_sp_bt.py
    python pipeline_edp_sp_bt.py --mes 05 --ano 2026 --pasta "//servidor/EDP_SP/2026-05"
    python pipeline_edp_sp_bt.py --so-ocr
    python pipeline_edp_sp_bt.py --so-digitacao
    python pipeline_edp_sp_bt.py --so-filtro
"""
...
try:
    from core.pipelines._visual import _p, _info, _ok, _fail, _warn, _sep, _banner, _rodar as _rodar_visual, _atualizar_master
except ModuleNotFoundError:
    from _visual import _p, _info, _ok, _fail, _warn, _sep, _banner, _rodar as _rodar_visual, _atualizar_master  # type: ignore[no-redef]

LOCAL_DIR = Path(__file__).resolve().parent.parent
SERVIDOR = Path("//10.10.250.21/Energia")

OCR_SCRIPT = LOCAL_DIR / "ocr" / "ocr_edp_sp_bt.py"
...
OCR_SAIDA_DIR = SERVIDOR / "ARQUIVOS ENZO" / "OCR EDP SP"
PIPELINE_SAIDA = SERVIDOR / "ARQUIVOS ENZO" / "EDP_SP_pipeline_saida"
DIGITADAS_DIR = Path("//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Digitadas")
PIPELINE_NOME = "EDP SP BT"
...
def _slug_resgate(pasta: Path, carimbos: list[str]) -> str:
    ...
    carimbos_limpos = [c.strip() for c in carimbos if c.strip()]
    if carimbos_limpos:
        partes.extend(carimbos_limpos[:3])
        if len(carimbos_limpos) > 3:
            partes.append(f"mais_{len(carimbos_limpos) - 3}")
...
def _xlsx_saida(mes: str, ano: str) -> Path:
    return OCR_SAIDA_DIR / f"ocr_edp_sp_BT_{mes}{ano}.xlsx"


def _xlsx_resgate(slug: str) -> Path:
    return OCR_SAIDA_DIR / "_resgates" / f"ocr_edp_sp_BT_{slug}.xlsx"
...
_AUDIT_PROBE_TENTATIVAS = 3
_AUDIT_PROBE_ESPERA_S = 1.0


def _aguardar_auditoria(pasta_saida: Path) -> Path | None:
    """Proba auditoria_resultados.csv com retry para tolerar latência SMB."""
    arq = pasta_saida / "auditoria_resultados.csv"
    for tentativa in range(1, _AUDIT_PROBE_TENTATIVAS + 1):
        try:
            if arq.exists():
                return arq
        except OSError:
            pass
        if tentativa < _AUDIT_PROBE_TENTATIVAS:
            time.sleep(_AUDIT_PROBE_ESPERA_S)
    return None
...
def etapa_digitacao(xlsx: Path, pasta_saida: Path) -> tuple[int, Path | None]:
    """Roda digitação e devolve (exit_code, caminho_confirmado_auditoria_ou_None)."""
    ...
    rc = _rodar_visual(f"DIGITACAO {PIPELINE_NOME} ({xlsx.name})", [PYTHON_EXE, str(DIGITACAO_SCRIPT)], env_extra=env_extra)
    audit_path = _aguardar_auditoria(pasta_saida)
    if audit_path is None:
        _warn(f"auditoria_resultados.csv nao encontrado em {pasta_saida} apos {_AUDIT_PROBE_TENTATIVAS} tentativas")
    return rc, audit_path


def etapa_filtro(
    root_pdfs: Path,
    pasta_saida: Path,
    auditoria_path: Path | None = None,
) -> int:
    if not FILTRO_SCRIPT.exists():
        _fail(f"Script de filtro nao encontrado: {FILTRO_SCRIPT}")
        return 1
    # Usa o caminho confirmado devolvido por etapa_digitacao; faz fallback
    # local para compatibilidade com chamadas --so-filtro que não passam o path.
    if auditoria_path is None:
        auditoria_path = _aguardar_auditoria(pasta_saida)
    if auditoria_path is None:
        _warn(f"auditoria_resultados.csv nao encontrado em {pasta_saida} - filtro pulado")
        return 0
    destino_existentes = Path(
        os.environ.get("NEO_FILTRO_DESTINO_EXISTENTES", str(DIGITADAS_DIR.parent / "Ja_existiam_no_Consen"))
    )
    env_extra = {
        "NEO_FILTRO_CSV":                str(auditoria_path),
        "NEO_FILTRO_ROOT":               str(root_pdfs),
        "NEO_FILTRO_DESTINO":            str(DIGITADAS_DIR),
        "NEO_FILTRO_DESTINO_EXISTENTES": str(destino_existentes),
    }
...
def main() -> int:
    ...
    if not args.so_ocr and not args.so_filtro:
        ...
        cod, audit_path = etapa_digitacao(xlsx, pasta_saida)
    ...
    if not args.so_ocr and not args.so_digitacao:
        etapa_filtro(pasta_pdfs, pasta_saida, auditoria_path=audit_path)
        try:
