...
# -*- coding: utf-8 -*-
"""
Correcao Equatorial GO BT por carimbo.

Fluxo:
1. Localiza os PDFs dos carimbos informados.
2. Reprocessa com o OCR Equatorial GO BT atualizado.
3. Corrige no Consen consumo, tributos, retencoes, bandeira e valores da fatura.
...
DEFAULT_SAIDA_DIR = "//10.10.250.21/Energia/ARQUIVOS ENZO/EQUATORIAL_GO_producao_saida/correcoes_bt_por_carimbo"
PDF_ROOTS_DEFAULT = (
...
CARIMBOS_PADRAO = ("2013036", "2013037", "2013038", "2013039")
...
try:
    ...
    from ocr.ocr_equatorial_go import processar_pdf
    ...
    from ocr_equatorial_go import processar_pdf  # type: ignore
...
def _correcoes_do_ocr(rec: dict[str, Any]) -> dict[str, str]:
    ...
    mapa = {
        "fatConFPontaIndRegistrado": rec.get("fatConFPontaIndRegistrado"),
        "fatConFPontaIndutivo": rec.get("fatConFPontaIndFaturado"),
        "fatConFPontaIndValorReais": rec.get("fatConFPontaIndValorReais"),
        "fatValorNFiscal": rec.get("fatValorNotaFiscal"),
        "fatValorFatura": rec.get("fatValorFatura"),
        ...
        "fatValBandeira": rec.get("fatValBandeira"),
        "fatMultas": rec.get("fatMultas"),
    }
    ...
    for campo_tela, valor in mapa.items():
        if valor_vazio(valor):
            continue
        try:
            if float(valor) == 0.0 and campo_tela not in {"fatTributoFederalPerc", "fatTributoFederalVal", "fatMultas"}:
                continue
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Correcao Equatorial GO BT por carimbo")
...
def _salvar_preparacao(linhas: list[dict[str, Any]]) -> None:
    ...
    path = SAIDA_DIR / "preparacao_correcao_equatorial_go_bt.csv"
    campos = ["carimbo", "arquivo", "status", "campos", "consumo", "valor_fatura", "valor_nf", "bandeira", "multa", "erro"]
            ...
            linhas_log.append(
                {
                    ...
                    "consumo": rec.get("fatConFPontaIndRegistrado"),
                    "valor_fatura": rec.get("fatValorFatura"),
                    "valor_nf": rec.get("fatValorNotaFiscal"),
                    "bandeira": rec.get("fatValBandeira"),
                    "multa": rec.get("fatMultas"),
                }
            )
