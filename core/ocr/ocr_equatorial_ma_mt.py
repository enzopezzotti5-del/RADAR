...
# -*- coding: utf-8 -*-
"""
ocr_equatorial_ma_mt.py
-----------------------
OCR de faturas MT Equatorial Maranhão → XLSX no schema CEMIG/Consen.
...
Uso:
    python ocr_equatorial_ma_mt.py --pasta "\\\\srv\\pasta\\MT" --saida ocr_ma_mt.xlsx
    python ocr_equatorial_ma_mt.py --pasta "..." --mes 06 --ano 2026

Env vars:
    EQUATORIAL_MA_MT_PASTA_PDF   — pasta-raiz com os PDFs
    EQUATORIAL_MA_MT_PASTA_SAIDA — pasta para o XLSX de saída
    EQUATORIAL_MA_CONC_COD       — código da concessionária no Consen
"""
...
PASTA_PDF_DEFAULT = Path(os.environ.get(
    "EQUATORIAL_MA_MT_PASTA_PDF",
    "//10.10.250.21/Energia/CONTASDEENERGIAELETRICA/BB/ENZO/Baixa Tensão/EQUATORIAL/MARANHÃO/MT",
))
PASTA_SAIDA_DEFAULT = Path(os.environ.get(
    "EQUATORIAL_MA_MT_PASTA_SAIDA",
    "//10.10.250.21/Energia/ARQUIVOS ENZO/OCR EQUATORIAL MA",
))
CONC_COD: str = os.environ.get("EQUATORIAL_MA_CONC_COD", "EQUATORIAL MA")
SISTEMA_MASTER = "EQUATORIAL MA MT"
MAX_WORKERS = 4
...
def _instalacao_formatada_do_nome(filename: str) -> str:
    ...
    Remove sufixo de data 'DD.MM' e qualquer texto adicional após ele:
      '644.233.016-03 - 26.07.pdf'       → '644.233.016-03'
      '829.435.016-23 - 26.07 media.pdf' → '829.435.016-23'
    """
    stem = Path(filename).stem
    return re.sub(r"\s*-+\s*\d{1,2}\.\d{2}.*$", "", stem).strip()
...
def _instalacao_do_texto(text: str) -> str:
    """Tenta extrair instalação do corpo da fatura."""
    upper = _norm(text)
    # Fallback: UC na seção UNIDADE CONSUMIDORA
...
def _extrair_mes_ref(text: str) -> dt.date | None:
    ...
    # Fallback: "EQUATORIAL MARANHAO ... MM/YYYY"
    m2 = re.search(
        r"EQUATORIAL\s+MARANH[^\n]+\s+(0[1-9]|1[0-2])/(20\d{2})\b",
        text, re.IGNORECASE,
    )
    if m2:
...
def _extrair_demanda_registrada_ma_mt(text: str) -> tuple[float, float]:
    """
...
def _extrair_retencoes(text: str) -> dict:
    ...
    # Padrão PI: "Tributo a Reter IRPJ 267,74"
    ...
    # Fallback: seção "ITENS FINANCEIROS" com labels diretos "IRPJ -267,74"
    if all(v == 0.0 for v in out.values()):
        # Tenta capturar retencoes em seção delimitada por ITENS FINANCEIROS
        secao_m = re.search(
...
def _extrair_itens(text: str) -> dict:
    ...
    for i, ln in enumerate(linhas):
        ...
        # ── Demanda ativa (não isenta, não ultrapassagem) ─────────────────────
        if (
        ...
        # ── Demanda isenta de ICMS: soma à demanda ───────────────────────────
        elif (
        ...
        # ── Energia FP: "Consumo Fora Ponta" / "TUSD Fora Ponta" ─────────────
        elif (
        ...
        # ── Energia Ponta: "Consumo Ponta" / "TUSD Ponta" ────────────────────
        elif (
            ("TUSD" in upper or "CONSUMO" in upper)
            and "PONTA" in upper
    ...
    dem_reg_fp, dem_reg_pta = _extrair_demanda_registrada_ma_mt(text)
    if dem_reg_fp:
    ...
    if instalacao and ref_str:
        ...
        try:
            indice.registrar(
                ...
                arquivo=filename,
                estado="MARANHÃO",
                concessionaria="Equatorial Maranhão",
            )
...
def processar_pdf(pdf_path: str | Path) -> dict:
    """Extrai dados de uma fatura MT Equatorial Maranhão."""
    ...
    try:
        ...
        # ── Desconto Fio B: MA MT não aplica ────────────────────────────────
        dados["fatDescontoFio"]    = 0.0
        dados["fatDescontoFioKWh"] = 0.0

        # ── Alíquotas de retenção (fixas por regulação federal) ──────────────
...
def parse_args() -> argparse.Namespace:
    hoje = dt.date.today()
    p = argparse.ArgumentParser(description="OCR Equatorial Maranhão MT → XLSX")
...
def main() -> int:
    ...
    if args.saida:
        ...
        xlsx_saida = PASTA_SAIDA_DEFAULT / f"ocr_equatorial_ma_MT_{mes}{ano}.xlsx"

    log.info("  OCR EQUATORIAL MARANHÃO — MT".center(60))
    log.info(f"  Pasta : {pasta}")
    log.info(f"  Saída : {xlsx_saida}")
